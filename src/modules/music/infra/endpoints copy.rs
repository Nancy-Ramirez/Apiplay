...
//*Función Update All */
#[put("/playlist/{id}")]
async fn update_playlist(info: web::Path<Info>, dto: web::Json<CreatePlaylist>) -> impl Responder {
    let mut playlists: Vec<Playlist> = vec![
        Playlist{
            name: "Basket Case".to_string(),
            songs: vec![],
        },
        Playlist{
            name: "Burning Love".to_string(),
            songs: vec![],
        },
        Playlist{
    ...
    playlists[info.id].name = dto.name.clone();
    web::Json(playlists[info.id].clone())
}

//*Función Patch  */
#[patch("/playlist/{id}")]
async fn partial_update_playlist(info: web::Path<Info>, dto: web::Json<PartialUpdatePlaylist>) -> impl Responder{
    let mut plylists: Vec<Playlist> = vec![
        Playlist{
            name: "Dancing Queen".to_string(),
            songs: vec![],
        },
        Playlist{
            name: "Beat it".to_string(),
            songs: vec![],
        },
        Playlist{
            name: "Livin'On A Prayer".to_string(),
    ...
    if let Some(play) = plylists.get_mut(info.id){
